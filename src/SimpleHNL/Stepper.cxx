...
TRandom3 * genie::HNL::HNLSelector::fRng;
bool genie::HNL::HNLSelector::fIsRngInit;

std::map< genie::HNL::HNLenums::HNLDecay_t, double > genie::HNL::HNLSelector::fCounterMap;

// empty the vector of counters
void ::genie::HNL::HNLSelector::emptyCounters( ){
    while( fCounterMap.size() > 0 ){ fCounterMap.erase( fCounterMap.begin() ); }
}

// populate the vector of counters
void ::genie::HNL::HNLSelector::fillCounters( std::map< genie::HNL::HNLenums::HNLDecay_t, double > gammaMap ){
	    if( fCounterMap.size() > 0 ) emptyCounters( );
	    if( !fIsRngInit ) initRandom( );
	    
	    for( std::map< genie::HNL::HNLenums::HNLDecay_t, double >::iterator
		     it = gammaMap.begin(); it != gammaMap.end(); ++it )
	      { fCounterMap.insert( std::pair< genie::HNL::HNLenums::HNLDecay_t, double > (
				      (*it).first, fRng->Uniform() ) ); }
	}

// inclusive method
std::vector< double > genie::HNL::HNLSelector::PropagateTilDecay( genie::HNL::SimpleHNL sh ){
...
// Exclusive stepper
void genie::HNL::HNLSelector::PropagateAndSelectChannel( genie::HNL::SimpleHNL sh ){

    const double COMlifetime = sh.GetCoMLifetime( );
    const double LABlifetime = sh.GetLifetime( );

    // first get the valid and the interesting channels with their widths
    const std::map< genie::HNL::HNLenums::HNLDecay_t, double > ValidMap = sh.GetValidChannels( );
    const std::map< genie::HNL::HNLenums::HNLDecay_t, double > InterestingMap = sh.GetInterestingChannels( );

    // TODO: exception for empty map!
    const int vSize = ValidMap.size();
    if( vSize == 0 ){ LOG( "SimpleHNL", pERROR ) << 
	  "genie::HNL::HNLSelector::PropagateAndSelectChannel:: " <<
    ...
    std::map< genie::HNL::HNLenums::HNLDecay_t, double > timeMap, tIntMap;

    for( std::map< genie::HNL::HNLenums::HNLDecay_t, double >::iterator it = fCounterMap.begin();
	 it != fCounterMap.end(); ++it ){
	// Exponential decay <-> N = floor( tau * ln( seed^(-1) ) / dt )
	const double tmpT = std::floor( LABlifetime *
					std::log( 1.0 / (*it).second ) /
					fdt ) * fdt;
	timeMap.insert( std::pair< genie::HNL::HNLenums::HNLDecay_t, double >( (*it).first, tmpT ) );

	// only insert in tIntMap if we have an interesting channel!
	if( InterestingMap.find( (*it).first ) != InterestingMap.end() ){
	    tIntMap.insert( std::pair< genie::HNL::HNLenums::HNLDecay_t, double >( (*it).first, tmpT ) ); }
    }

    // first channel wins.
    double t1 = 99999.99, t2 = 99999.99; genie::HNL::HNLenums::HNLDecay_t c1;
    for( std::map< genie::HNL::HNLenums::HNLDecay_t, double >::iterator it = timeMap.begin();
	 it != timeMap.end(); ++it ){
	if( (*it).second < t1 ){ t1 = (*it).second; } } // t1 from all valid times
    for( std::map< genie::HNL::HNLenums::HNLDecay_t, double >::iterator it = tIntMap.begin();
	 it != tIntMap.end(); ++it ){
